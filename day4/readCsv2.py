...
# 2.每个测试用例的路径不同，所以path应该作为参数传入到这个方法（read）中
...
def read(file_name):
    ...
    # try...finally...也可以保证程序中间发生异常，文件最后也可以关闭，但是finally语法可读性比较差，写起来比较容易出错，一般都用with语句代替finally
    ...
    # file.close()也不执行了，这时，文件仍然不能关闭
    # 所以用过用with...as...语句实现文件的关闭
    #file.close()
...
if __name__ == '__main__':
    ...
    # 所以首先要找到当前文件的路径
    # os 操作系统, path 路径, dir 是directory 目录
    #__file__ 是python内置的变量，指的是当前文件
    # current_file_path = os.path.dirname(__file__)
    # print(current_path)
    # 我们真正想要的路径是csv文件的路径
